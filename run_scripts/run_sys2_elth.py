...
from crosslinking_codes2 import crosslink_sys2
...
generate_lattice_sys2(num_chains=2000, chain_length=16, num_C_units=1000, output_file='lattice.dat', min_distance=0.8, box_size=250)

# Relax the lattice
os.system('{} -in in.relax'.format(run_lammps))

# Crosslinking the system
crosslink_sys2(relaxed_file='relax.dat', output_file='test_ck.dat', lmp_command='{} -in in.anneal'.format(run_lammps), percentage_ck_A=1, percentage_ck_B=1, functionality_A=3, functionality_B=3, functionality_C=4, ck_dis_A=1.2, ck_dis_B=1.2, probability_AC=0.8, probability_B=0.4, save_snapshots=True)
