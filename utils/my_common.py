...
def pbc_distance(coor1, coor2, box):
    """
    Calculate the distance between two points in a periodic box.

    Parameters:
    coor1 (array-like): The coordinates of the first point.
    coor2 (array-like): The coordinates of the second point.
    box (array-like): The dimensions of the periodic box.

    Returns:
    float: The distance between the two points.
    """
    delta = np.abs(coor1.reshape(-1,3) - coor2.reshape(-1,3))
    delta[:,0] = np.where(delta[:,0] > 0.5 * box[0,0], delta[:,0] - box[0,0], delta[:,0])
    delta[:,1] = np.where(delta[:,1] > 0.5 * box[1,1], delta[:,1] - box[1,1], delta[:,1])
    delta[:,2] = np.where(delta[:,2] > 0.5 * box[2,2], delta[:,2] - box[2,2], delta[:,2])
